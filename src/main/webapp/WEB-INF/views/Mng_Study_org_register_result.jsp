...
<body id="page-top"  onload="doSomething()">
...
<%
     ...
     String l_str_o_nm = request.getParameter("str_o_nm"); //학습조직명
     String l_saubu = request.getParameter("saubu"); //사업부
     String l_deptname = request.getParameter("deptname");//부서
     String l_b_kind = request.getParameter("b_kind"); //분류
     String l_leader_nm = request.getParameter("leader_nm"); //학습조직리더
     String l_st_member= request.getParameter("st_member"); //구성원
     String l_str_o_remark= request.getParameter("str_o_remark"); //설명


    
        System.out.println("학습조직명   dsfdsfdse 들어와요"+l_str_o_nm);

    //    out.println("사업부   저장"+l_saubu);
        System.out.println("사업부  dsfdsfdse 들어와요"+l_saubu);


      //  out.println("부서   들어옹ㅁ 저장"+l_deptname);
        System.out.println("부서   dsfdsfdse 들어와요"+l_deptname);
         ...
         try {
             ...
             // SQL 쿼리 실행
             String sql = "INSERT INTO H_MINOR (MAJOR_CD,MINOR_CD,MINOR_NM,USE_FLG) VALUES (?,?,?,?)";
             pstmt = conn.prepareStatement(sql);
             pstmt.setString(1, l_str_o_nm);
             pstmt.setString(2, l_saubu);
             pstmt.setString(3, l_deptname);
             pstmt.setString(4, l_b_kind);

              // 쿼리 실행
              pstmt.executeUpdate();
        //        out.println("데이터베이스에 저장되었습니다.");
                System.out.println("데이터베이스에 --정상적으로---- 저장되었습니다.");
             } catch (Exception e) {
          //      out.println("오류 발생: " + e.getMessage());
                System.out.println("데이터베이스에 --비비비 비 정상적으로---- 저장되었습니다.");
             } finally {
                 // 연결 종료
